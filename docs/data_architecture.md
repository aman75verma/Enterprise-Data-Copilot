...
*   **Agents**: Internal support staff who own or resolve tickets. Tickets reference `agents.id` through `agent_id`, allowing workload and resolution analytics by agent.
...
4.  **Storage in `doc_chunks`**: Each processed chunk is stored in the `doc_chunks` table with `source_file`, `content`, `embedding VECTOR(384)`, `title`, `product`, `section`, `url`, `chunk_index`, and `last_updated`.
5.  **Metadata**: Alongside the vector, metadata is stored to improve retrieval relevance:
...
The application stores chat history separately from business data and documentation so the Copilot can preserve session context and support debugging, audit, and evaluation workflows.

### Tables
*   **Conversations**: One row per chat session. The `id` is a UUID generated by Postgres and `created_at` records when the session began.
*   **Turns**: One row per message in a conversation. Each turn references `conversations.id` through `conversation_id` and stores `role`, `content`, optional `tool_calls`, optional `tool_name`, `latency_ms`, `token_usage`, and `created_at`.

### Why These Tables Exist
`conversations` and `turns` make Copilot behavior auditable. They let the system replay a support interaction, inspect which tools were called, measure latency, and track token usage without mixing runtime chat logs into customer, ticket, or document tables.
